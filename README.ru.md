...
- `-c CHECK`, `--check CHECK`: Указать набор параметров или пресетов для сравнения. Может быть комбинацией [epic,full,data,fast,tree,name,dirname,filename,date,size,bytes,firstbytes,lastbytes,count,dircount,filecount,hash]. Вы можете указать несколько значений, разделенных запятыми. Например, `--check size,name`
- `-i IGNORE`, `--ignore IGNORE`: Указать набор параметров или пресетов для игнорирования при сравнении. Может быть комбинацией [epic,full,data,fast,tree,name,dirname,filename,date,size,bytes,firstbytes,lastbytes,count,dircount,filecount,hash]. Вы можете указать несколько значений, разделенных запятыми. Например, `--ignore date`
...
##### Фильтры процессинга
Предварительная фильтрация для процессинга.
- `--include-empty`,
  `--include-empty-dirs`,
  `--include-empty-files`: Обрабатывать пустые директории и/или файлы.
- `--exclude PATTERNS`,
  `--exclude-dirs DIR_PATTERNS`,
  `--exclude-files FILE_PATTERNS`: Исключать директории и файлы из сравнения, которые соответствуют заданным шаблонам. Можно использовать шаблоны, например, `*.bak` или `/dir/**`

#### Финальная обработка
Фильтр результатов. Данные фильтры отвечают за финальную фильтрацию, никак не влияют на процесс поиска дубликатов, но влияют на то, к каким файлам и директориям будут применены действия.

- `-b`, `--brief`: Отображать краткую статистику и не выполнять никаких дальнейших действий.
- `--no-dirs`: Не выводить и не применять действий к директориям в финале
- `--no-files`: Не выводить и не применять действий к файлам в финале
- `--dups-count DUPS_COUNT`,
  `--dups-dirs-count DUPS_DIRS_COUNT`,
  `--dups-files-count DUPS_FILES_COUNT`: Минимальное количество дубликатов директорий и/или файлов в группе
- `--min-size MINSIZE`,
  `--max-size MAXSIZE`,
  `--min-dir-size MIN_DIR_SIZE`,
  `--max-dir-size MAX_DIR_SIZE`,
  `--min-file-size MIN_FILE_SIZE`,
  `--max-file-size MAX_FILE_SIZE`: Эти аргументы задают минимальные и максимальные размеры для файлов и директорий в финальной выборке. Размеры должны указываться в виде строки с числом и единицей измерения. Например, `--min-size 10KB` или `--max-dir-size 2GiB`. Данный фильтр не исключает файлы и директории из сравнения, а только фильтрует финальный результат
...
Найти все файлы и директории размером от 1GiB с одинаковым именем, которые дублируются от пяти раз. Вывести тайминги выполнения и пути в относительном виде:
```bash
